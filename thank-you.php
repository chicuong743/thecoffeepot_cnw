...
function send_order_mail($con, $email, $name, $phone, $payment_mode,$shipping,$tracking_no) {
    ...
    $shipping_query = "SELECT price FROM shipping_unit WHERE id = '$shipping'";
        ...
        if ($shipping_result && mysqli_num_rows($shipping_result) > 0) {
            ...
            $shipping_price = 150000; // Mặc định là 100k nếu không tìm thấy phương thức giao hàng
        }
    if ($orderResult && mysqli_num_rows($orderResult) > 0) {
        ...
        $customerBody = "
            ...
            <p><b>Tổng đơn hàng:</b> " . number_format($totalPrice, 0, ',', '.') . " VND</p>
            <br><hr>
    ...
    require_once "C:/xampp/htdocs/bannoithat/PHPMailer/PHPMailer.php";
    require_once "C:/xampp/htdocs/bannoithat/PHPMailer/SMTP.php";
    require_once "C:/xampp/htdocs/bannoithat/PHPMailer/Exception.php";
...
if(isset($_GET['payment_mode']) && $_GET['payment_mode']=="Thanh toán khi nhận hàng"){

}else if(isset($_GET['payment_mode']) && $_GET['payment_mode']=="Thanh toán qua PayPal"){

}else if(isset($_GET['payment_mode']) && $_GET['payment_mode']=="Thanh toán qua MoMo"){
    // Kiểm tra xem các tham số cần thiết có tồn tại trong URL không
...
<div class="py-3 bg-primary">
    <div class="container">
        <h6 class="text-white">
            ...
            <a class="text-white" href="">Đặt hàng thành công</a>
        </h6>
    </div>
</div>

<div class="container">
    <h1 class="mt-2">Cảm ơn bạn đã đặt hàng!</h1>
    ...
    <p>Đơn hàng của bạn đã được xử lý <span style="color: red; font-size: larger; font-weight: bolder;">Thành Công</span>. Chúng tôi sẽ liên hệ với bạn trong thời gian sớm nhất để xác nhận và giao hàng.</p>
    <p>Nếu bạn có bất kỳ <span style="color: red; font-size: larger; font-weight: bolder;">Câu Hỏi</span> nào, vui lòng liên hệ với chúng tôi qua <span style="color: red; font-size: larger; font-weight: bolder;">Email</span> hoặc <span style="color: red; font-size: larger; font-weight: bolder;">Số điện thoại</span>.</p>

    <a href="index.php" class=""> << Quay lại trang chủ </a>
</div>
